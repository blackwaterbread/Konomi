...
describe("image db integration", () => {
  ...
  it("finds exact duplicate groups and respects ignored duplicate state", async () => {
    ...
    const duplicateService = createDuplicateService({
      ...
      ignoredDuplicates: {
        ...
        register: async (paths) => {
          for (const p of paths) ignoredPaths.add(p);
        },
        forget: async (p) => {
          ignoredPaths.delete(p);
        },
        list: async () => [...ignoredPaths],
        clear: async () => {
          const n = ignoredPaths.size;
          ignoredPaths.clear();
          return n;
        },
      },
    ...
    const duplicate =
      await duplicateService.findDuplicateForIncomingPath(incomingPath);
    ...
    ignoredPaths.add(incomingPath);
    await expect(
      duplicateService.findDuplicateForIncomingPath(incomingPath),
    ).resolves.toBe(null);

    ignoredPaths.delete(incomingPath);
    await expect(
      duplicateService.findDuplicateForIncomingPath(incomingPath),
