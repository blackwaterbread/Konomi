...
afterEach(async () => {
  const { closePromptsDB } = await import("../../../main/lib/prompts-db");
  closePromptsDB();
  delete process.env.KONOMI_PROMPTS_DB_PATH;
  delete process.env.KONOMI_USER_DATA;
...
describe("prompts-db", () => {
  ...
  it("computes fallback prompt-tag stats when metadata thresholds are absent", async () => {
    const dbPath = createPromptDb({
      meta: {
        schema_version: "4",
        tag_count_total: "10",
    ...
    const { suggestPromptTags } = await import("../../../main/lib/prompts-db");
    const result = suggestPromptTags({
      prefix: "tag-",
      limit: 3,
    });
